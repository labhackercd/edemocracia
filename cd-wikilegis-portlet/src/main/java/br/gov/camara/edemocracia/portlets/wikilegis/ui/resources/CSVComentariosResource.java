...
public class CSVComentariosResource extends WebResource {

	private static final long serialVersionUID = 1L;
	private static final Log LOG = LogFactoryUtil.getLog(CSVComentariosResource.class);
	private final long groupId;

	public CSVComentariosResource() {
	...
	protected void setHeaders(WebResponse response) {
		...
		response.setAttachmentHeader("comentarios-" + groupId + ".csv");
		...
		AbstractResourceStream stream = new AbstractResourceStream() {
			...
			public InputStream getInputStream() throws ResourceStreamNotFoundException {
				if (bais == null) {
					...
					try {
						ExportacaoCSV.getComentariosCSV(groupId, writer);
					} catch (SystemException e) {
						throw new ResourceStreamNotFoundException();
					} catch (PortalException e) {
						LOG.error("Erro ao exportar comentários", e);
						throw new ResourceStreamNotFoundException();
