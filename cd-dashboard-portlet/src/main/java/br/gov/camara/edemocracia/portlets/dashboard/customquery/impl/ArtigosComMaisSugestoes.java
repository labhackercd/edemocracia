...
public class ArtigosComMaisSugestoes extends QueryExecutor {
	...
	protected List<RecursoDTO> listar(Connection connection, Configuracao config, long companyId) throws SQLException {
		...
		String sql = SQLBuilder.buildSQLArtigosWikilegisComMaisSugestoes(config);
		...
		try {
			...
			for (RecursoDTO recurso : retorno) {
				String titulo = recurso.getTitulo();
				if (titulo.length() > 120) { 
					recurso.setTitulo(titulo.substring(0, 100) + " ...");
				}
			}
	...
	public CacheKey getChaveCache() {
		return CacheKey.SUGESTOES_WIKILEGIS;
	}
