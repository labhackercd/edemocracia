...
public class WikisComMaiorParticipacao extends QueryExecutor {
	...
	protected List<RecursoDTO> listar(Connection connection, Configuracao config, long companyId) throws SQLException {
		...
		long wikiPageClassId = ClassNameLocalServiceUtil.getClassNameId(WikiPage.class);

		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List<RecursoDTO> retorno = new ArrayList<RecursoDTO>();
		String sql = SQLBuilder.buildSQLWikisComMaiorParticipacao(config);

		try {
			pstmt = connection.prepareStatement(sql);
			pstmt.setLong(1, companyId);
			pstmt.setLong(2, wikiPageClassId);
