...
if (layoutSet != null) {
	...
	try {
		Group group = GroupLocalServiceUtil.getGroup(PortalUtil.getCompanyId(request), GroupConstants.GUEST);
		Layout layout = LayoutLocalServiceUtil.getFriendlyURLLayout(group.getGroupId(), false, NOT_FOUND_LAYOUT_FRIENDLY_URL);
		redirect = PortalUtil.getPortalURL(request) + PortalUtil.getPathContext() + PortalUtil.getPathFriendlyURLPublic() + group.getFriendlyURL() + layout.getFriendlyURL();
