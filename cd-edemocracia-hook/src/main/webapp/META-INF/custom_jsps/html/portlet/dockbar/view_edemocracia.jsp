<div class="dockbar" data-namespace="<portlet:namespace />" id="dockbar">
	<ul class="aui-toolbar">
		...
		<c:if test="<%= !group.isControlPanel() && (!group.hasStagingGroup() || group.isStagingGroup()) && (GroupPermissionUtil.contains(permissionChecker, group.getGroupId(), ActionKeys.ADD_LAYOUT) || LayoutPermissionUtil.contains(permissionChecker, layout, ActionKeys.UPDATE) || (layoutTypePortlet.isCustomizable() && layoutTypePortlet.isCustomizedView() && LayoutPermissionUtil.contains(permissionChecker, layout, ActionKeys.CUSTOMIZE))) %>">
		...
		<c:if test="<%= !group.isControlPanel() && (themeDisplay.isShowLayoutTemplatesIcon() || themeDisplay.isShowManageSiteMembershipsIcon() || themeDisplay.isShowPageSettingsIcon() || themeDisplay.isShowSiteContentIcon() || themeDisplay.isShowSiteMapSettingsIcon() || themeDisplay.isShowSiteSettingsIcon() || themeDisplay.isShowControlPanelIcon()) %>">
		...
		<c:if test="<%= !group.isControlPanel() && LayoutPermissionUtil.contains(permissionChecker, layout, ActionKeys.UPDATE) || (layoutTypePortlet.isCustomizable() && layoutTypePortlet.isCustomizedView() && LayoutPermissionUtil.contains(permissionChecker, layout, ActionKeys.CUSTOMIZE)) %>">
	...
	<ul class="aui-toolbar user-toolbar">
		<c:if test="<%= user.hasMySites() %>">
			<li class="my-sites has-submenu" id="<portlet:namespace />mySites">
				<a class="menu-button" href="javascript:;">
					<span>
						<img src="/e-democracia-theme/images/custom/ico_comunidades.png" alt="Comunidades"> 
					</span>
				</a>

				<div class="aui-menu my-sites-menu aui-overlaycontext-hidden" id="<portlet:namespace />mySitesContainer">
					<div class="aui-menu-content">
						<%@ include file="/html/portlet/dockbar/my_sites.jsp" %>
					</div>
				</div>
			</li>
		</c:if>
...
<%
	String urlMyProfile;
	List<Layout> myLayouts = LayoutLocalServiceUtil.getLayouts(themeDisplay.getUser().getGroup().getGroupId(), false, LayoutConstants.DEFAULT_PARENT_LAYOUT_ID);
	if (myLayouts.isEmpty()) {
		myLayouts = LayoutLocalServiceUtil.getLayouts(themeDisplay.getUser().getGroup().getGroupId(), true, LayoutConstants.DEFAULT_PARENT_LAYOUT_ID);
	}
	if (!myLayouts.isEmpty()) {
		urlMyProfile = PortalUtil.getLayoutURL(myLayouts.get(0), themeDisplay);
	} else {
		Group guest = GroupLocalServiceUtil.getGroup(themeDisplay.getCompanyId(), GroupConstants.GUEST);
		myLayouts = LayoutLocalServiceUtil.getLayouts(guest.getGroupId(), false, LayoutConstants.DEFAULT_PARENT_LAYOUT_ID);
		if (myLayouts.isEmpty()) {
			myLayouts = LayoutLocalServiceUtil.getLayouts(guest.getGroupId(), true, LayoutConstants.DEFAULT_PARENT_LAYOUT_ID);
		}
		if (!myLayouts.isEmpty())
			urlMyProfile = PortalUtil.getLayoutURL(myLayouts.get(0), themeDisplay);
		else
...
%>
		<li class="user-avatar <%= themeDisplay.isImpersonated() ? "impersonating-user has-submenu" : "" %>" id="<portlet:namespace />userAvatar">
			<span class="user-links <%= themeDisplay.isImpersonated() ? "menu-button": "" %>">
				<aui:a cssClass="user-portrait" data-controlPanelCategory="<%= PortletCategoryKeys.MY %>" href="<%= urlMyProfile %>" title="manage-my-account">
					<img alt="<%= HtmlUtil.escape(user.getFullName()) %>" src="<%= HtmlUtil.escape(user.getPortraitURL(themeDisplay)) %>" />
				</aui:a>

				<span class="user-fullname" title="<%= HtmlUtil.escape(user.getFullName()) %>"><%= HtmlUtil.escape(user.getFullName()) %></span>
