...
String labelNewMessage = preferences.getValue(MBLabelKeys.NEW_MESSAGE,MBLabelKeys.NEW_MESSAGE);
...
<liferay-ui:header
	...
	title='<%= (message == null) ? labelNewMessage : message.getSubject() %>'
/>
...
<aui:form action="<%= editMessageURL %>" enctype='<%= attachments ? "multipart/form-data" : "" %>' method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveMessage(false);" %>'>
	...
	<aui:fieldset>
		...
		<c:if test="<%= message == null %>">
			<%
				String classHidden = themeDisplay.getPermissionChecker().isCommunityAdmin(scopeGroupId) ? StringPool.BLANK : "aui-helper-hidden" ;
			%>
			<aui:field-wrapper label="permissions" cssClass="<%=classHidden%>">
