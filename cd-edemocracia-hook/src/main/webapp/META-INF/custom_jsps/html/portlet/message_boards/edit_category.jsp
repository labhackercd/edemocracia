...
String labelForumNewCategory = preferences.getValue(MBLabelKeys.FORUM_NEW_CATEGORY,MBLabelKeys.FORUM_NEW_CATEGORY);
String labelForumParentCategory = preferences.getValue(MBLabelKeys.FORUM_PARENT_CATEGORY,MBLabelKeys.FORUM_PARENT_CATEGORY);
String labelMergeWithParentCategory = preferences.getValue(MBLabelKeys.MERGE_WITH_PARENT_CATEGORY,MBLabelKeys.MERGE_WITH_PARENT_CATEGORY);
%>

<liferay-ui:header
	backURL="<%= redirect %>"
	title='<%= (category != null) ? category.getName() : labelForumNewCategory %>'
/>
...
<aui:form action="<%= editCategoryURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveCategory();" %>'>
	...
	<aui:fieldset>
		<aui:field-wrapper label="<%=labelForumParentCategory %>">
			...
			<aui:input label="<%=labelMergeWithParentCategory %>" name="mergeWithParentCategory" type="checkbox" />
		</aui:field-wrapper>
